...
// File paths
const inputFilePath = path.join(__dirname, "countries_ADM1.geojson");
const outputFilePath = path.join(__dirname, "countryStateMapping.json");
...
try {
  ...
  console.log("Successfully read input JSON");
  console.log(`Found ${Object.keys(jsonData).length} top-level keys`);

  // Process the JSON data
  const step1Data = extractRawData(jsonData);

  const processedData = getCountryStateMapping(
    [
      "SAU",
      "RUS",
      "CAN",
...
function extractRawData(data) {
  // For now, just return a copy of the input data
  const processedData = data.features.map((feature) => feature.properties);

  return processedData;
}

function getCountryStateMapping(countries, jsonData) {
  const output = {};

  for (country of countries) {
    const c = countryCodes.find((i) => i.alpha3 === country).alpha2;

    const countryData = jsonData.filter((s) => s.shapeGroup === country);

    output[c] = {};
    countryData.forEach((item) => {
      output[c][item.shapeName] = item.shapeName;
    });
  }
